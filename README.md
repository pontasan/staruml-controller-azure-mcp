# StarUML Controller MCP Server — Azure Architecture Diagram

An MCP (Model Context Protocol) server specialized for **Azure Architecture Diagrams**. Enables AI assistants like Claude to programmatically create and manage Azure architecture diagrams — Azure elements, groups, services, and connectors in [StarUML](https://staruml.io/).

<p align="center">
  <img src="images/main.jpg" alt="Azure E-Commerce Architecture created with staruml-controller-azure-mcp" width="800">
</p>

<p align="center"><em>Azure E-Commerce Architecture — created entirely via MCP tools</em></p>

## Architecture

<p align="center">
  <img src="images/architecture.svg" alt="Architecture: Claude ↔ MCP (stdio) ↔ staruml-controller-azure-mcp ↔ HTTP REST API ↔ StarUML" width="800">
</p>
...
```bash
git clone https://github.com/pontasan/staruml-controller-azure-mcp.git
cd staruml-controller-azure-mcp
npm install && npm run build
```
...
5. All Azure Architecture Diagram tools become available via MCP
...
```json
{
  "mcpServers": {
    "staruml-azure": {
      "command": "node",
      "args": ["/absolute/path/to/staruml-controller-azure-mcp/dist/index.js"],
...
```bash
claude mcp add staruml-azure -e STARUML_PASSWORD=<password> -- node /absolute/path/to/staruml-controller-azure-mcp/dist/index.js
```
...
### Azure Architecture Diagram Tools

The core of this server — full CRUD for every Azure Architecture Diagram resource.

| Resource | Tools |
|---|---|
| **Diagrams** | `azure_list_diagrams`, `azure_create_diagram`, `azure_get_diagram`, `azure_update_diagram`, `azure_delete_diagram` |
| **Elements** | `azure_list_elements`, `azure_create_element`, `azure_get_element`, `azure_update_element`, `azure_delete_element` |
| **Connectors** | `azure_list_connectors`, `azure_create_connector`, `azure_get_connector`, `azure_update_connector`, `azure_delete_connector` |
